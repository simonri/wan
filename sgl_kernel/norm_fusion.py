...
def apply_norm_cta(
  ...
  tidx: cutlass.Int32,
  tXrX: cute.Tensor,
  tWrW: cute.Tensor | None,
  tBrB: cute.Tensor | None,
...
def apply_rmsnorm_cta(
  num_warps: cutlass.Int32 | cutlass.Constexpr,
  tidx: cutlass.Int32,
  tXrX: cute.Tensor,
  tWrW: cute.Tensor | None,
  D: cutlass.Int32 | cutlass.Constexpr,
...
def apply_layernorm_cta(
  ...
  # Normalize
  tNrN = cute.make_fragment_like(tXrX)
  if cutlass.const_expr(isinstance(tWrW, cute.Tensor) and isinstance(tBrB, cute.Tensor)):
    tNrN.store(((tXrX.load() - mean) * factor * tWrW.load() + tBrB.load()).to(tNrN.element_type))
  else:
...
def tensor_slice_for_bsfd(
  ...
  S: cutlass.Int32 | cutlass.Constexpr,
  D: cutlass.Int32 | cutlass.Constexpr,
