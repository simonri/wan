...
class FlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin, BaseScheduler):
  ...
  def set_timesteps(
    ...
    if self.config.use_karras_sigmas:
      ...
      sigmas_tensor = torch.from_numpy(sigmas_array).to(dtype=torch.float32)
      sigmas_tensor = self._convert_to_exponential(in_sigmas=sigmas_tensor, num_inference_steps=num_inference_steps)
      sigmas_array = sigmas_tensor.numpy()
  ...
  def step(
    ...
    if batch is not None and batch.rollout:
      ...
      prev_sample = self.flow_sde_sampling(batch, model_output, sample, current_sigma, next_sigma, generator)
    else:
  ...
  def _time_shift_exponential(self, mu: float, sigma: float, t: torch.Tensor | np.ndarray) -> torch.Tensor | np.ndarray:
