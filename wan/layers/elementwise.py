...
from wan.kernels.scale_shift import fuse_scale_shift_kernel
...
class MulAdd(CustomOp):
  ...
  def forward_native(self, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, k: int = 0) -> torch.Tensor:
  ...
  def forward_cuda(self, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, k: int = 0):
    return fuse_scale_shift_kernel(a, b, c, scale_constant=k)

  def forward_xpu(self, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, k: int = 0):
