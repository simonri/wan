...
class ScaleResidualNormScaleShift:
  ...
  def kernel(
    ...
    def norm(x, weight, bias):
      return apply_norm_cta(self.norm_type, self.num_warps, tidx, x, weight, bias, self.D, eps)
...
def to_fake_cute_args(t: torch.Tensor):
  if isinstance(t, torch.Tensor):
    ...
    fake_t = cute.runtime.make_fake_tensor(dtype, shape, stride, memspace=cute.AddressSpace.gmem, assumed_align=32)
    return fake_t
