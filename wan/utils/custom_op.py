...
def register_custom_op(
  ...
  **extra_kwargs,
  ...
  assert expected_kwarg_keys >= extra_kwarg_keys, f"Unexpected extra kwargs: {extra_kwarg_keys - expected_kwarg_keys}"

  has_out_shape = "out_shape" in extra_kwargs
  has_fake_impl = "fake_impl" in extra_kwargs
  assert not (has_out_shape and has_fake_impl), "Only one of `out_shape` or `fake_impl` should be provided."
...
class CustomOpWrapper:
  ...
  def real_impl(self) -> Callable:
    if self._impl is None:
      if not hasattr(torch.ops.sglang, self.op_name):
        from wan.utils.common import direct_register_custom_op
  ...
  def fake_impl(self) -> Callable:
    ...
    def fake_impl(*args, **kwargs):
      ...
      try:
        return torch.empty_like(bound.args[out_shape] if isinstance(out_shape, int) else bound.arguments[out_shape])
  ...
  *,
  ...
  Example usage::
    ...
    trtllm_fp8_block_scale_moe = register_custom_op_from_extern(
      ...
      computed_args={
        "tune_max_num_tokens": lambda hidden_states, **kw: next_power_of_2(hidden_states.shape[0]),
      },
    )
  ...
  if computed_args:
    ...
    new_params = [p for param_name, p in original_sig.parameters.items() if param_name not in computed_args]
