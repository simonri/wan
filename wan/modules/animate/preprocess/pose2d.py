...
class SimpleOnnxInference(object):
  def __init__(self, checkpoint, device='cuda', reverse_input=False, **kwargs):
    ...
    if device.type == 'cuda':
      ...
      providers = [("CUDAExecutionProvider", {"device_id": device[-1:] if device[-1] in [
                    str(_i) for _i in range(10)] else "0"}), "CPUExecutionProvider"]
    else:
    ...
    self.input_resolution = self.session.get_inputs(
    )[0].shape[2:] if not reverse_input else self.session.get_inputs()[0].shape[2:][::-1]
...
class Yolo(SimpleOnnxInference):
  ...
  def postprocess(self, output, shape_raw, cat_id=[1]):
    ...
    if outputs.shape[-1] == 6:
      ...
      width = (w * x_factor)
      height = (h * y_factor)
      boxes = np.concatenate([left, top, width, height],
  ...
  def forward(self, img, shape_raw, **kwargs):
    ...
    outputs = self.session.run(
        None, {self.session.get_inputs()[0].name: img})[0]
    person_results = [[{'bbox': np.array(
        [0., 0., 1.*shape_raw[i][1], 1.*shape_raw[i][0], -1]), 'track_id': -1}] for i in range(len(outputs))]

    for i in range(len(outputs)):
      self.postprocess_threading(
          outputs, shape_raw, person_results, i, **kwargs)
    return person_results
