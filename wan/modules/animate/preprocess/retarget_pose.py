...
def get_scaled_pose(canvas, src_canvas, keypoints, keypoints_hand, bone_ratio_list, delta_ground_x, delta_ground_y,
  ...
  for idx in range(len(keypoints)):
    ...
    keypoints[idx] = [keypoints[idx][0] * src_W,
                      keypoints[idx][1] * src_H, keypoints[idx][2]]
  ...
  if rescale_keypoints[4] == None and rescale_keypoints[7] == None:
    ...
    if keypoints[4][0] != None and left_hand[0].x != -1:
      left_hand_root_offset = np.array(
          (keypoints[4][0] - left_hand[0].x * src_W,  keypoints[4][1] - left_hand[0].y * src_H))
      left_hand_delta += left_hand_root_offset

    if keypoints[7][0] != None and right_hand[0].x != -1:
      right_hand_root_offset = np.array(
          (keypoints[7][0] - right_hand[0].x * src_W, keypoints[7][1] - right_hand[0].y * src_H))
      right_hand_delta += right_hand_root_offset

    dis_left_hand = ((keypoints[4][0] - left_hand[0].x * src_W) **
                     2 + (keypoints[4][1] - left_hand[0].y * src_H) ** 2) ** 0.5
    dis_right_hand = ((keypoints[7][0] - left_hand[0].x * src_W) **
                      2 + (keypoints[7][1] - left_hand[0].y * src_H) ** 2) ** 0.5

    if dis_left_hand > dis_right_hand:
      right_hand_new = get_handpose_meta(
          left_hand, right_hand_delta, src_H, src_W)
      left_hand_new = get_handpose_meta(
          right_hand, left_hand_delta, src_H, src_W)
    else:
      left_hand_new = get_handpose_meta(
          left_hand, left_hand_delta, src_H, src_W)
      right_hand_new = get_handpose_meta(
          right_hand, right_hand_delta, src_H, src_W)
...
def rescale_skeleton(H, W, keypoints, bone_ratio_list):
  ...
  for idx in range(len(rescale_keypoints)):
    ...
    rescale_keypoints[idx] = [rescale_keypoints[idx]
                              [0] * W, rescale_keypoints[idx][1] * H]
    ...
    for idx, limb in enumerate(part):
      ...
      if keypoint1 != None and keypoint2 is None:
        ...
        if ref_length != 0:
          ...
          skeleton['keypoints_body'][k2_index -
                                     1][0] = skeleton['keypoints_body'][k1_index - 1][0]
          skeleton['keypoints_body'][k2_index -
                                     1][1] = skeleton['keypoints_body'][k1_index - 1][1] + ref_length / H
  return skeleton
...
def write_to_poses(data_to_json, none_idx, dst_shape, bone_ratio_list, delta_ground_x, delta_ground_y, rescaled_src_ground_x, body_flag, scale_min):
  ...
  for id in tqdm(range(length)):
    ...
    # get hand keypoints
    keypoints_hand = {'left': data_to_json[id]['keypoints_left_hand'],
    ...
    frame_info = get_scaled_pose((height, width), (src_height, src_width), new_keypoints, keypoints_hand,
                                 bone_ratio_list, delta_ground_x, delta_ground_y, rescaled_src_ground_x, body_flag, id, scale_min)
...
def calculate_scale_ratio(skeleton, skeleton_edit, scale_ratio_flag):
  if scale_ratio_flag:
    ...
    headw = max(skeleton['keypoints_body'][0][0], skeleton['keypoints_body'][14][0], skeleton['keypoints_body'][15][0], skeleton['keypoints_body'][16][0], skeleton['keypoints_body'][17][0]) - \
        min(skeleton['keypoints_body'][0][0], skeleton['keypoints_body'][14][0], skeleton['keypoints_body']
            [15][0], skeleton['keypoints_body'][16][0], skeleton['keypoints_body'][17][0])
    headw_edit = max(skeleton_edit['keypoints_body'][0][0], skeleton_edit['keypoints_body'][14][0], skeleton_edit['keypoints_body'][15][0], skeleton_edit['keypoints_body'][16][0], skeleton_edit['keypoints_body'][17][0]) - \
        min(skeleton_edit['keypoints_body'][0][0], skeleton_edit['keypoints_body'][14][0], skeleton_edit['keypoints_body']
            [15][0], skeleton_edit['keypoints_body'][16][0], skeleton_edit['keypoints_body'][17][0])
...
def retarget_pose(src_skeleton, dst_skeleton, all_src_skeleton, src_skeleton_edit, dst_skeleton_edit, threshold=0.4):
  ...
  if src_skeleton['keypoints_body'][0] != None and src_skeleton['keypoints_body'][10] != None and src_skeleton['keypoints_body'][13] != None and \
    ...
    scale_min = np.sqrt(src_skeleton['height'] * src_skeleton['width']) / \
        np.sqrt(dst_skeleton['height'] * dst_skeleton['width'])
  ...
  if use_edit_for_base:
    ...
    if src_skeleton_edit['keypoints_body'][0] != None and src_skeleton_edit['keypoints_body'][10] != None and src_skeleton_edit['keypoints_body'][13] != None and \
      ...
      scale_min_edit = np.sqrt(src_skeleton_edit['height'] * src_skeleton_edit['width']) / np.sqrt(
          dst_skeleton_edit['height'] * dst_skeleton_edit['width'])
    ...
    dst_skeleton_edit['height'] = int(
        dst_skeleton_edit['height'] * scale_min_edit)
    dst_skeleton_edit['width'] = int(
        dst_skeleton_edit['width'] * scale_min_edit)
  ...
  src_body_flag = check_full_body(
      src_skeleton_ori['keypoints_body'], threshold)
  ...
  if body_flag == 'full_body':
    ...
    if dst_skeleton['keypoints_body'][18] != None and dst_skeleton['keypoints_body'][19] != None:
      right_foot_mid = (dst_skeleton['keypoints_body'][10]
                        [1] + dst_skeleton['keypoints_body'][19][1]) / 2
      left_foot_mid = (dst_skeleton['keypoints_body'][13]
                       [1] + dst_skeleton['keypoints_body'][18][1]) / 2
    ...
    dst_ground_x = (dst_skeleton['keypoints_body'][8][0] + dst_skeleton['keypoints_body'][11][0]) * dst_skeleton[
        'width'] / 2
    rescaled_src_ground_x = (
        rescaled_src_skeleton_ori[8][0] + rescaled_src_skeleton_ori[11][0]) / 2
...
def get_retarget_pose(tpl_pose_meta0, refer_pose_meta, tpl_pose_metas, tql_edit_pose_meta0, refer_edit_pose_meta):
  ...
  for key, value in tpl_pose_meta0.items():
    if type(value) is np.ndarray:
      if key in ['keypoints_left_hand', 'keypoints_right_hand']:
        value = value * \
            np.array([[tpl_pose_meta0["width"], tpl_pose_meta0["height"], 1.0]])
  ...
  for key, value in refer_pose_meta.items():
    if type(value) is np.ndarray:
      if key in ['keypoints_left_hand', 'keypoints_right_hand']:
        value = value * \
            np.array([[refer_pose_meta["width"], refer_pose_meta["height"], 1.0]])
      if not isinstance(value, list):
        value = value.tolist()
    ...
    for key, value in tql_edit_pose_meta0.items():
      if type(value) is np.ndarray:
        if key in ['keypoints_left_hand', 'keypoints_right_hand']:
          value = value * \
              np.array(
                  [[tql_edit_pose_meta0["width"], tql_edit_pose_meta0["height"], 1.0]])
        if not isinstance(value, list):
          value = value.tolist()
    ...
    for key, value in refer_edit_pose_meta.items():
      if type(value) is np.ndarray:
        if key in ['keypoints_left_hand', 'keypoints_right_hand']:
          value = value * \
              np.array(
                  [[refer_edit_pose_meta["width"], refer_edit_pose_meta["height"], 1.0]])
        if not isinstance(value, list):
          value = value.tolist()
      refer_edit_pose_meta[key] = value

  retarget_tpl_pose_metas = retarget_pose(
      tpl_pose_meta0, refer_pose_meta, tpl_pose_metas_new, tql_edit_pose_meta0, refer_edit_pose_meta)
  ...
  for meta in retarget_tpl_pose_metas:
    ...
    pose_meta.kps_body = np.array(meta["keypoints_body"])[
        :, :2] * (width, height)
