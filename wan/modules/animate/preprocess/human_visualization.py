...
def split_pose2d_kps_to_aa(kp2ds: np.ndarray) -> List[np.ndarray]:
  ...
  kp2ds_body = (
      ...
      + kp2ds[[0, 5, 6, 8, 10, 5, 7, 9, 12, 14,
               16, 11, 13, 15, 2, 1, 4, 3, 18, 21]]
  ) / 2
...
def draw_aapose_by_meta(img, meta: AAPoseMeta, threshold=0.5, stick_width_norm=200, draw_hand=True, draw_head=True):
  ...
  kp2ds_lhand = np.concatenate(
      [meta.kps_lhand, meta.kps_lhand_p[:, None]], axis=1)
  kp2ds_rhand = np.concatenate(
      [meta.kps_rhand, meta.kps_rhand_p[:, None]], axis=1)
  pose_img = draw_aapose(img, kp2ds, threshold, kp2ds_lhand=kp2ds_lhand, kp2ds_rhand=kp2ds_rhand,
                         stick_width_norm=stick_width_norm, draw_hand=draw_hand, draw_head=draw_head)
  return pose_img
...
def draw_hand_by_meta(img, meta: AAPoseMeta, threshold=0.5, stick_width_norm=200):
  ...
  kp2ds_rhand = np.concatenate(
      [meta.kps_rhand, meta.kps_rhand_p[:, None]], axis=1)
  pose_img = draw_aapose(img, kp2ds, threshold, kp2ds_lhand=kp2ds_lhand, kp2ds_rhand=kp2ds_rhand,
                         stick_width_norm=stick_width_norm, draw_hand=True, draw_head=False)
  return pose_img
...
def draw_aaface_by_meta(img, meta: AAPoseMeta, threshold=0.5, stick_width_norm=200, draw_hand=False, draw_head=True):
  ...
  pose_img = draw_M(img, kp2ds, threshold, kp2ds_lhand=None, kp2ds_rhand=None,
                    stick_width_norm=stick_width_norm, draw_hand=draw_hand, draw_head=draw_head)
  return pose_img
...
def draw_aanose_by_meta(img, meta: AAPoseMeta, threshold=0.5, stick_width_norm=100, draw_hand=False):
  ...
  # kp2ds_lhand = np.concatenate([meta.kps_lhand, meta.kps_lhand_p[:, None]], axis=1)
  # kp2ds_rhand = np.concatenate([meta.kps_rhand, meta.kps_rhand_p[:, None]], axis=1)
  pose_img = draw_nose(img, kp2ds, threshold, kp2ds_lhand=None, kp2ds_rhand=None,
                       stick_width_norm=stick_width_norm, draw_hand=draw_hand)
  return pose_img
...
def draw_pcd(pcd_list, save_path=None):
  ...
  for _idx, _pcd in enumerate(pcd_list):
    ax.scatter(_pcd[:, 0], _pcd[:, 1], _pcd[:, 2],
               c=color_list[_idx], marker="o")
...
def draw_traj(metas: List[AAPoseMeta], threshold=0.6):
  ...
  colors = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0],
      ...
      255, 0, 85], [100, 255, 50], [255, 100, 50],
      # foot
      [200, 200, 0],
      [100, 100, 0]
  ]
  ...
  for _idx, ((k1_index, k2_index)) in enumerate(limbSeq):
    ...
    vis = (kp_body_p[:, k1_index] > threshold) * \
        (kp_body_p[:, k2_index] > threshold) * 1
  ...
  for i in range(key_points_list.shape[-2]):
    ...
    for idx, point in enumerate(points):
      x, y, vis = point
      if vis == 1:
        cv2.circle(_image_vis, (x, y), stickwidth,
                   sample_colors[idx], thickness=-1)
