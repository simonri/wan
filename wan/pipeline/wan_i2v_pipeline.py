...
class WanImageToVideoPipeline(PipelineBase):
  ...
  def create_pipeline_stages(self, config: WanI2VConfig):
    ...
    self.add_stage(TextEncodingStage(text_encoder=text_encoder, tokenizer=tokenizer))

    self.vae = Wan2_1_VAE(vae_pth=config.dit_config.vae_checkpoint, device=self.device)
